...
void storagepolicytest :: setUp (void)
{
    ...
    //Protect from CORBA::Release from ComponentStorageSmartPtr
    comp = MockComponent::_duplicate(comp);
}

void storagepolicytest :: tearDown (void)
{
    mcs->releaseComponent(compname.c_str());
    delete mcs;
}
...
void storagepolicytest :: Destroy_nilhandleTest (void)
{
    ...
    foo.Destroy();
    CPPUNIT_ASSERT_EQUAL (mcs->map->find(compname,mc), 0);
}
...
void storagepolicytest :: Destroy_cannotreleaseexTest (void)
{
    ...
    //Protect from CORBA::Release from ComponentStorageSmartPtr
    badcomp = MockComponent::_duplicate(badcomp);
