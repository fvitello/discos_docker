...
namespace Logging {
    ...
    Logger::~Logger()
    {
        ...
        {
          if (ACE_Singleton<Logger_ptr, ACE_Recursive_Thread_Mutex>::instance()->globalLogger_m)
          {
            log(logItem);
          }
        }
        else
          if (loggerName_m == BaseLog::STATIC_LOGGER_NAME)
          {
            if (ACE_Singleton<Logger_ptr, ACE_Recursive_Thread_Mutex>::instance()->staticLogger_m)
            {
              log(logItem);
            }
          }
        else
          if (loggerName_m == BaseLog::ANONYMOUS_LOGGER_NAME)
          {
            if (ACE_Singleton<Logger_ptr, ACE_Recursive_Thread_Mutex>::instance()->anonymousLogger_m)
            {
              log(logItem);
            }
          }
